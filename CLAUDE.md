...
- **`scripts/delayed.js`** — deferred work (delayed phase, ~3s after LCP). Currently an empty stub.
- **`scripts/martech.js`** — `loadLaunch()` (default export), called from `loadLazy` in `scripts.js` (**lazy** phase, not delayed, so the consent banner / tags surface promptly while staying off the critical render path). Loads the **Adobe Experience Platform Launch (tags)** embed via `loadScript` (from `aem.js`), picking the Development / Staging / Production embed code by hostname (`DEV_HOSTS` `localhost`/`127.0.0.1`/`local.telenet.be`→dev, `*.aem.page`→staging, `*.aem.live` + `PROD_DOMAINS`→prod). `local.telenet.be` is a hosts-file alias for the dev server used so OneTrust's `telenet.be` consent cookie can be stored locally (consent can't persist on `localhost`/`*.aem.page`/`*.aem.live` — the cookie domain won't match); it's checked **before** the prod rule so it doesn't count as prod. Launch in turn loads ContentSquare / OneTrust / Analytics (configured in the Launch UI, consent-gated). `loadLaunch()` also wires (via document-level event delegation) any link/button labelled **"Cookievoorkeuren aanpassen"** (or `href` `#cookievoorkeuren`/`#ot-settings`) to open the OneTrust preference centre (`OneTrust.ToggleInfoDisplay()`).
