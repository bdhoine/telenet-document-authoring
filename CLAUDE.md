...
- **DA experimentation rail** — `scripts/sidekick.js` toggles the da.live `exp.js` rail on the sidekick `custom:experimentation` event; the bottom of `scripts.js` also loads `exp.js` on `?daexperiment` (and DA preview on `?dapreview`). Needs an `experimentation` plugin entry in the project sidekick config. Authors define experiments in a page's **Metadata** block (`Experiment`, `Experiment Variants`, optionally `Audience`/`Campaign`).
